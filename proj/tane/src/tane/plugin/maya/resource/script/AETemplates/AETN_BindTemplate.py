...
def refresh_all_aetemplates(force=False):
    ...
    widgets = get_maya_window().findChildren(AETN_Bind_InSourceTemplate) or []
...
class AETN_Bind_InSourceTemplate(QtWidgets.QWidget):
    ...
    def __init__(self, node, parent=None):
        ...
        super(AETN_Bind_InSourceTemplate, self).__init__(parent)
...
def build_qt_widget(lay, node):
    ...
    widget = AETN_Bind_InSourceTemplate(node)
...
def update_qt_widget(layout, node):
    ...
    if ptr is not None:
        ...
        for c in range(maya_layout.count()):
            ...
            if isinstance(widget, AETN_Bind_InSourceTemplate):
                force = True
