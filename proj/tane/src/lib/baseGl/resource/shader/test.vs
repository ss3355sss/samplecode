...
void main()
{
    VNormal    = normalize(gl_NormalMatrix * mat3(kXformMtx) * InNormal);
    VPosition  = vec3(gl_ModelViewMatrix * kXformMtx * vec4(InPoint, 1.0));

    VTexCoord  = InTexCoord;
    VColor     = InColor;
    VWireColor = InWireColor;

    gl_Position = gl_ModelViewProjectionMatrix * kXformMtx * vec4(InPoint, 1.0);
